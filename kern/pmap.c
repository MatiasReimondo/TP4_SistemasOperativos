...
mem_init(void)
{
	...
	//Solo le doy [KSTACKTOP-KSTKSIZE, KSTACKTOP) para que, si hay overflow, no se pise la memoria
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
		...
		if (!(size % PTSIZE) && !(va % PTSIZE) && !(pa % PTSIZE)){
			while (pos < size){
				//Page directory de 4MB
				pgdir[PDX(va + pos)] = (pa + pos) | (perm | PTE_P);
				pgdir[PDX(va + pos)] = pgdir[PDX(va + pos)] | perm | PTE_P | PTE_PS;
				pos+= PTSIZE;
			}
		} else{
			for (pos = 0; pos < size; pos+=PGSIZE){
